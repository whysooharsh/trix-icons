...
/**
 * Story: the gear engages - a partial clockwise detent turn with a brief hold at
 * the detent, then release back to rest. Out-and-back rather than snap-back so no
 * geometric symmetry of the source mark is assumed. Deliberately partial: refresh
 * owns the full 360deg cycle; this reads "adjust", not "reload".
 */
...
const DURATION_TURN = 0.28;
const HOLD_AT_DETENT = 0.09;

// A single detent of adjustment, not a full revolution.
const DETENT_DEG = 30;
...
export const SettingsIcon = forwardRef<AnimatedIconHandle, AnimatedIconProps>(
  function SettingsIcon(
    ...
    const startAnimation = useCallback(async () => {
      ...
      // 1. Engage: turn to the detent
      await ctrl.start({
        rotate: DETENT_DEG,
        transition: { duration: DURATION_TURN, ease: EASE_STANDARD },
      });

      // 2. Release: brief hold, then settle back to rest
      await ctrl.start({
        rotate: 0,
        transition: { delay: HOLD_AT_DETENT, duration: DURATION_TURN, ease: EASE_STANDARD },
      });
